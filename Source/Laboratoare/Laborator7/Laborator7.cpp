...
std::stack<int> spellQueue;

int lumos[] = { GLFW_KEY_A, GLFW_KEY_M, GLFW_KEY_I, GLFW_KEY_X, GLFW_KEY_A, GLFW_KEY_M, GLFW_KEY_SPACE, GLFW_KEY_S, GLFW_KEY_O, GLFW_KEY_M, GLFW_KEY_U, GLFW_KEY_L};
...
{
	bool applySpell = false;
	if (key == 257) {
		int cont = 0;
		while (!spellQueue.empty() || !applySpell) {
			int c = spellQueue.top();
